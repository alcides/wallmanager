import os
from os.path import join, dirname, abspath

#SET TRUE ON WALL
PRODUCTION = 'SENSEWALL' in os.environ

if PRODUCTION:
    os.environ['DJANGO_SETTINGS_MODULE'] = 'settings-prod'
else:
    os.environ['DJANGO_SETTINGS_MODULE'] = 'settings'

import sys
sys.path.append('..')
sys.path.append('../webmanager')

# Returns the absolute path to this file directory joined
# with whatever directories given as arguments
def relative(*x):
    return join(abspath(dirname(__file__)), *x)


APPS_REPOSITORY_PATH = relative('apps/')
APPS_BOOT_FILENAME = 'boot.bat'

##PROXY CONFIGURATIONS
PROXY_UDP_IP   = '127.0.0.1'
PROXY_RECEIVING_PORT = 6000
PROXY_SENDING_PORT_ONE = 6001
PROXY_SENDING_PORT_TWO = 3333

## UI
if PRODUCTION:
    MAINWINDOW_SIZE = (2020,768)
else:
    MAINWINDOW_SIZE = (1200,700)
MAINWINDOW_POSITION = (0,0)


TOPBAR_SIZE = (MAINWINDOW_SIZE[0], 148)
TOPBAR_POSITION = (0,MAINWINDOW_SIZE[1]-TOPBAR_SIZE[1])

CATEGORYLIST_SIZE = (170, MAINWINDOW_SIZE[1]-TOPBAR_SIZE[1])
CATEGORYLIST_POSITION = (0,0)
CATEGORYLIST_FRICTION = 5
CATEGORYLIST_LABEL_SIDES_MARGIN = 18
CATEGORYLIST_LABEL_FONT_SIZE = 15

APPSLIST_POSITION = (CATEGORYLIST_SIZE[0],0)
APPSLIST_SIZE = (MAINWINDOW_SIZE[0] - CATEGORYLIST_SIZE[0], MAINWINDOW_SIZE[1]-TOPBAR_SIZE[1])
APPSLIST_NUMBER_OF_LINES = 3
APPSLIST_FRICTION = 5
APPSLIST_PADDING_X = 100
APPSLIST_PADDING_Y = 40
APPSLIST_POPUP_DURATION = 5.0
APPSLIST_STAR_ONLY_WHEN_ORDER_BY_RATING = True # If false star is always showed
APPSLIST_BTN_SIZE = (100,100)
APPSLIST_BTN_IMAGE_SIZE = (80,80)
APPSLIST_BTN_FONT_SIZE = 10
APPSLIST_BTN_POPUPS_PER_BTN = 1
APPSLIST_BTN_POPUPS = 1

VOTEPOPUP_SIZE = (500, 180)
VOTEPOPUP_POSITION = (int((MAINWINDOW_SIZE[0] - VOTEPOPUP_SIZE[0]) / 2),
                      int((MAINWINDOW_SIZE[1] - VOTEPOPUP_SIZE[1]) / 2))
VOTEPOPUP_QUESTION = 'What do you think about this application?'
VOTEPOPUP_BTN_SIZE = (200, 75)
VOTEPOPUP_BTN_LIKE_COLOR = (0, 0.5, 0.05, 1) # Dark Green
VOTEPOPUP_BTN_DISLIKE_COLOR = (1, 0, 0, 1) # Red

HELPPOPUP_SIZE = (700,700)
HELPPOPUP_POSITION = (int((MAINWINDOW_SIZE[0] - HELPPOPUP_SIZE[0]) / 2),
                      int((MAINWINDOW_SIZE[1] - HELPPOPUP_SIZE[1]) / 2))

APPPOPUP_SIZE = (270,255)

INACTIVITY_POOL_INTERVAL = 5
UNAVAILABLE_PROJECTORS_TIME = 2
TIME_TO_CHECK_PROJECTORS = 10

# LAUCHING APPLICATION SETTINGS
MAX_ATTEMPTS = 3
SLEEP_SECONDS_BETWEEN_ATTEMPTS = 2

NATIVE_APP_NAMES = [' Community Core Vision ','Atalho para launcher.py', 'WatchDog']

# WINDOWS MANAGER
SET_FULLSCREEN = 3

# PROJECTORS INTERFACE
SATURDAY = 5

# COVER WINDOW
COVER_WINDOW_RESUME_TIME = 10.0


## GESTURE
GESTURE_ACCEPTANCE_MARGIN = 0.80
GESTURE_KILLER = ""


