from os.path import join, dirname, abspath

import sys

sys.path.append('..')
from webmanager.settings import APPS_MAX_LOG_ENTRIES


# Returns the absolute path to this file directory joined
# with whatever directories given as arguments
def relative(*x):
    return join(abspath(dirname(__file__)), *x)


APPS_REPOSITORY_PATH = relative('apps/')
APPS_BOOT_FILENAME = 'boot.bat'

##PROXY CONFIGURATIONS
UDP_IP   = '127.0.0.1'
RECEIVING_PORT = 6000
SENDING_PORT_ONE = 6001
SENDING_PORT_TWO = 3333
MESSAGE  = "Hello, World!"


#UI
SCATTER_POSITION = (50, 50)
SCATTER_SIZE = (900,550)
PADDING = 5
CATEGORY_GRID_POSITION = (PADDING, PADDING)
<<<<<<< HEAD:mtmenu/settings.py
<<<<<<< HEAD:mtmenu/settings.py
CATEGORY_GRID_SIZE = (130, SCATTER_SIZE[1] - CATEGORY_GRID_POSITION[0] - 75)                                     
APPS_GRID_POSITION = (CATEGORY_GRID_POSITION[0] + CATEGORY_GRID_SIZE[0] + PADDING, PADDING)
APPS_GRID_SIZE = (SCATTER_SIZE[0] - CATEGORY_GRID_SIZE[0] - CATEGORY_GRID_POSITION[0] - 2*PADDING, CATEGORY_GRID_SIZE[1])
=======
=======
>>>>>>> d6119fe... Top left logo.:mtmenu/settings.py
CATEGORY_GRID_SIZE = (150, SCATTER_SIZE[1] - CATEGORY_GRID_POSITION[0] - PADDING - 60)                                     
APPS_GRID_POSITION = (CATEGORY_GRID_POSITION[0] + CATEGORY_GRID_SIZE[0] + 5, PADDING)
APPS_GRID_SIZE = (CATEGORY_GRID_SIZE[1] + 150, CATEGORY_GRID_SIZE[1])
>>>>>>> d6119fe... Top left logo.:mtmenu/settings.py
POPUP_SIZE = (215,160)
POPUP_POSITION = (200, 200) 


WALL_DEFAULT_WIDTH = 1024 * 2
WALL_DEFAULT_HEIGHT = 748

ACCEPTANCE_MARGIN = 0.90

KILLER_GESTURE = ""

